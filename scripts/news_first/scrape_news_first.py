...
def fetch_news_data(
    category_id: int = 83, page: int = 2, count: int = 5
) -> Dict[str, Any]:
...
def save_articles_by_category(articles: List[NewsArticle], category_id: int):
    ...
    print(
        f"Saved {new_articles_count} new articles to: data/news_first/{category_name}/"
    )
    return saved_files


def fetch_and_save_all_categories(
    pages_per_category: int = 2, articles_per_page: int = 10
):
    ...
    print(f"\n{'=' * 50}")
    print(
        f"SUMMARY: Total new files saved across all categories: {len(all_saved_files)}"
    )
    print(f"{'=' * 50}")
